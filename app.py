...
lineaccesstoken = 'SaZZ5buWInh+BcHfdSW6Uor+WslFAqOxBZeDCDVuTOOUbUPSM+Bdi7GAWP+uROX4sdF81rz3ttrQt460zsuiVYjsQb0X0sXgd8Wp7vYRQTAq+GOwHh7V+dPWglsCRV5U0jICkjncuqxbnDe6oCQKPAdB04t89/1O/w1cDnyilFU='
...
def event_handle(event,json_line):
    ...
    if msgType == "text":
        ...
        if (msg == "สวัสดี") :
            replyObj = TextSendMessage(text="ธุจ้า")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "กินข้าวยังคะ") :
            replyObj = TextSendMessage(text="ยังค่า ซื้อให้กินโหน่ย")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "เด็กดื้อต้องโดนอะไรค้า") :
            replyObj = TextSendMessage(text="โดนค้อนทุบหัว ตุยเย่ไปเลออ")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "ปลาอะไรใหญ่กว่าปลาหยุด") :
            replyObj = TextSendMessage(text="ปลาหยุดชุปแป้งทอด ผ่ามพาม")
            line_bot_api.reply_message(rtoken, replyObj)
        else :
            headers = request.headers
            json_headers = ({k:v for k, v in headers.items()})
            json_headers.update({'Host':'dialogflow.cloud.google.com'})
            url = "https://dialogflow.cloud.google.com/v1/integrations/line/webhook/94cfa2ed-569b-426a-a173-579e8ab1c0fc"
